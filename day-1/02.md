...
# Shell environment: commands, aliases, and variables

**Last update**: 20260904-5
...
### 2. Shell environment <a href="#environment" id="environment"></a>
...
For most cases of interest, we are executing commands in the terminal in the following way:
...
Let us now scrutinize the above general syntax for command execution term by term:
...
* By default, aliases are available only in the terminal in which you have defined them. But this can be easily circumvented by modifying the special configuration file `.bashrc` and/or an additional customary file `.bash_aliases` &mdash; to be clarified later;
...
*   Aliases can overwrite the name of the existing **Linux** or **Bash** command, and aliases will have the higher precedence in execution. In this case, you can execute the overwritten command by prepending backslash `\` to its name, i.e. with `\commandName`. For instance:
    ...
    In this context, we have _escaped_ the alias definition of **ls** with the special symbol backslash `\`;
