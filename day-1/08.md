...
On a side note, it is important to realize that
...
are not logically equivalent, because in the former the C is executed if B failed, whereas in the latter C is not executed if B failed.
...
#### General case <a href="#general" id="general"></a>
...
#### Integers <a href="#integers" id="integers"></a>
...
#### Files and directories <a href="#files-directories" id="files-directories"></a>
...
Now the user got a prompt message `Password:` in the terminal and his input is not shown on the screen as he types it, but it was stored silently in the variable **UserPassword** (any other name for the variable is perfectly fine as well!). Within your subsequent code you can programmatically check the content of the **UserPassword** variable. If you remove the read permission on the file where you are doing those checks, you have obtained a very simple-minded mechanism for handling passwords, etc.
...
The file's content is redirected to the loop with `<` operator at the end of the loop (redirections operators are discussed in detail later).
...
The special syntax `:-` is used to set the default values of shell variables. With the above implementation, we have the following behavior at execution, from example directory `/home/abilandz/Test`:
...
Use with care, though, the shell built-in command **shift** as the code quickly becomes unreadable and challenging to maintain, if **shift** is overused.
