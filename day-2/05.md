...
The closely related command is **pgrep**, which can also interpret regular expressions in the command names. For the above example, we can use:
...
After using command **fg**, the background job is continuing to run in the foreground and is, therefore, taking over the control over the terminal. the general syntax of the **fg** command is the same as for **bg** command:
