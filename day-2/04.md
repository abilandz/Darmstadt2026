...
* `someFiles(s)` &mdash; one or more files which **awk** will parse line-by-line automatically. During parsing, each line of a file becomes a line of input to **awk**, on which `PATTERN { ACTION }` sequences are tested and executed.
